...
namespace LexicalAnalyzer
{
    ...
    {
        Regex endOfLineRegex = new Regex(@"\}|\}", RegexOptions.Compiled);
        ...
        public IEnumerable<Token> Tokenize(string source)
        {
            ...
            while (currentIndex < source.Length)
            {
                ...
                {
                    throw new Exception(string.Format("Invalid Token in '{0}' at index {1} (line {2}, column {3}).", source[currentIndex], currentIndex, currentLine, currentColumn));
                }
                else
                {
